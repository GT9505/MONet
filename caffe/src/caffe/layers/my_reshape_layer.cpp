...
#include "caffe/layers/my_reshape_layer.hpp"

namespace caffe {

template <typename Dtype>
void MyReshapeLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  CHECK_NE(top[0], bottom[0]) << this->type() << " Layer does not "
      "allow in-place computation.";
  inferred_axis_ = -1;
  copy_axes_.clear();
  const BlobShape& top_blob_shape = this->layer_param_.my_reshape_param().shape();
  
  int my_shape[4];
  my_shape[0]=bottom[1]->num();
  my_shape[1]=bottom[1]->channels();
  my_shape[2]=bottom[1]->height();
  my_shape[3]=bottom[1]->width();
  
  const int top_num_axes = top_blob_shape.dim_size();
  constant_count_ = 1;
  for (int i = 0; i < top_num_axes; ++i) {
    const int top_dim = my_shape[i];
    if (top_dim == 0) {
...
template <typename Dtype>
void MyReshapeLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  const int input_start_axis = this->layer_param_.my_reshape_param().axis();
  ...
  const int num_axes = this->layer_param_.my_reshape_param().num_axes();
  ...
  for (int i = 0; i < num_new_axes; ++i) {
    top_shape[top_shape_index++] = my_shape[i];
  }
