...
class coco(imdb):
    ...
    def _load_coco_annotation(self, index):
        ...
        for obj in objs:
            ...
            if obj['area'] > 0 and x2-2 > x1 and y2-2 > y1:
        ...
        #segmentations = np.zeros((num_objs,height,width),dtype=np.uint8)
        segmentations = []
        iscrowds = np.zeros((num_objs), dtype=np.uint8)
    ...
    def _coco_results_one_category(self, boxes, cat_id):
        ...
        for im_ind, index in enumerate(self.image_index):
            ...
            scores = np.around(dets[:, -1],4)
            xs = np.around(dets[:, 0],1)
            ys = np.around(dets[:, 1],1)
            ws = np.around(dets[:, 2] - xs + 1,1)
            hs = np.around(dets[:, 3] - ys + 1,1)
    ...
    def _write_coco_results_file(self, all_boxes, res_file):
        ...
        for cls_ind, cls in enumerate(self.classes):
            ...
            if cls_ind>=1 and cls_ind<=80:
                results1.extend(self._coco_results_one_category(all_boxes[cls_ind],
                                                           coco_cat_id))
            if cls_ind>=81 and cls_ind<=80:
                results2.extend(self._coco_results_one_category(all_boxes[cls_ind],
                                                           coco_cat_id))
            if cls_ind>=81 and cls_ind<=80:
                results3.extend(self._coco_results_one_category(all_boxes[cls_ind],
                                                           coco_cat_id))
            if cls_ind>=81 and cls_ind<=100:
                results4.extend(self._coco_results_one_category(all_boxes[cls_ind],
                                                           coco_cat_id))
        print 'Writing results json to {}'.format(res_file)
        res_file_a=os.path.splitext(res_file)[0]
        res_file_b=os.path.splitext(res_file)[1]
        res_file1= res_file_a+'_1'+res_file_b
        res_file2= res_file_a+'_2'+res_file_b
        res_file3= res_file_a+'_3'+res_file_b
        res_file4= res_file_a+'_4'+res_file_b
        json.encoder.FLOAT_REPR = lambda x: format(x, '.3f')
        with open(res_file1, 'w') as fid1:
            json.dump(results1, fid1)
        with open(res_file2, 'w') as fid2:
            json.dump(results2, fid2)
        with open(res_file3, 'w') as fid3:
            json.dump(results3, fid3)
        with open(res_file4, 'w') as fid4:
            json.dump(results4, fid4)
    ...
    def evaluate_detections(self, all_boxes, output_dir):
        ...
        res_file_a=os.path.splitext(res_file)[0]
        res_file_b=os.path.splitext(res_file)[1]
        res_file= res_file_a+'_1'+res_file_b
    ...
    def _mask_coco_results_one_category(self, masks, cat_id):
        results = []
        for im_ind, index in enumerate(self.image_index):
            mask = masks[im_ind]
            if mask == []:
            ...
            for k in range(len(mask)/2):
                results.extend(
                    [{'image_id' : index,
                    'category_id' : cat_id,
                    'segmentation' : mask[2*k],
                    'score' : mask[2*k+1].astype(np.float)}] )

        return results

    def _mask_write_coco_results_file(self, all_masks, res_file):
        # [{"image_id": 42,
        #   "category_id": 18,
        #   "bbox": [258.15,41.29,348.26,243.78],
        #   "score": 0.236}, ...]
        ...
        for cls_ind, cls in enumerate(self.classes):
            ...
            if cls_ind>=1 and cls_ind<=40:
                results1.extend(self._mask_coco_results_one_category(all_masks[cls_ind],
                                                           coco_cat_id))
            if cls_ind>=41 and cls_ind<=80:
                results2.extend(self._mask_coco_results_one_category(all_masks[cls_ind],
                                                           coco_cat_id))
            if cls_ind>=81 and cls_ind<=80:
                results3.extend(self._mask_coco_results_one_category(all_masks[cls_ind],
                                                           coco_cat_id))
            if cls_ind>=81 and cls_ind<=100:
                results4.extend(self._mask_coco_results_one_category(all_masks[cls_ind],
                                                           coco_cat_id))
        print 'Writing results json to {}'.format(res_file)
        res_file_a=os.path.splitext(res_file)[0]
        res_file_b=os.path.splitext(res_file)[1]
    ...
    def evaluate_masks(self, all_masks, output_dir):
        res_file = osp.join(output_dir, ('masks_' +
                                         self._image_set +
                                         self._year +
                                         '_results'))
        if self.config['use_salt']:
            res_file += '_{}'.format(str(uuid.uuid4()))
        res_file += '.json'
        self._mask_write_coco_results_file(all_masks, res_file)
        # Only do evaluation on non-test sets
