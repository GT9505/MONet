...
class RoIDataLayer(caffe.Layer):
    ...
    def _shuffle_classes_roidb_inds(self, i):
        """Randomly permute the training roidb."""
        if cfg.TRAIN.ASPECT_GROUPING:
            widths = np.array([r['width'] for r in self._classes_roidb[i]])
            heights = np.array([r['height'] for r in self._classes_roidb[i]])
            horz = (widths >= heights)
            vert = np.logical_not(horz)
            horz_inds = np.where(horz)[0]
            vert_inds = np.where(vert)[0]
            ...
            row_perm = np.random.permutation(np.arange(inds.shape[0]))
            inds = np.reshape(inds[row_perm, :], (-1,))
            self._perm_cla[i] = inds
        else:
            self._perm_cla[i] = np.random.permutation(np.arange(len(self._classes_roidb[i])))
        self._cur_cla[i] = 0

    def _shuffle_classes_inds(self):
	"""Randomly choose a class index."""
        self._cla = np.random.permutation(np.arange(len(self._classes_roidb)))
        self._cla_i = 0

    def _get_next_classes_minibatch_inds(self, i):
        """Return the roidb indices for the next minibatch."""
        if self._cur_cla[i] + cfg.TRAIN.IMS_PER_BATCH >= len(self._classes_roidb[i]):
            self._shuffle_classes_roidb_inds(i)

        db_inds = self._perm_cla[i][self._cur_cla[i]:self._cur_cla[i] + cfg.TRAIN.IMS_PER_BATCH]
        self._cur_cla[i] += cfg.TRAIN.IMS_PER_BATCH
        return db_inds

    def _get_next_classes_inds(self):
        if self._cla_i == len(self._classes_roidb):
            self._shuffle_classes_inds()

        cla_inds = self._cla[self._cla_i]
        self._cla_i += 1
        return cla_inds

    def _sample_classes(self):
        """Return the lists of different classes."""
        temp = [[] for j in range(self._num_classes)]
        self._classes_roidb = [[] for j in range(self._num_classes - 1)]
        for i in xrange(self._num_classes):
            for r in self._roidb:
                if i in r['gt_classes']:
                    temp[i].append(r)
        self._classes_roidb = temp[1:]
    ...
    def set_roidb(self, roidb, gpu_id=0):
        ...
        if cfg.TRAIN.LABEL_SHUFFLING:
            ...
            self._perm_cla = [[0] for j in range(len(self._classes_roidb))]
            self._cur_cla = [0 for j in range(len(self._classes_roidb))]
            self._shuffle_classes_inds()
            for i in range(len(self._classes_roidb)):
                self._shuffle_classes_roidb_inds(i)
    ...
    def setup(self, bottom, top):
        ...
        idx = 0
        if cfg.TRAIN.IMAGE_BASE_SIZE > 1:
            top[idx].reshape(cfg.TRAIN.IMS_PER_BATCH, 3,
                cfg.TRAIN.IMAGE_BASE_SIZE, cfg.TRAIN.IMAGE_BASE_SIZE)
        else:
            top[idx].reshape(cfg.TRAIN.IMS_PER_BATCH, 3,
        ...
        if cfg.TRAIN.HAS_RPN:
            ...
            # mask annotations
            if cfg.TRAIN.MASK_ON:
                top[idx].reshape(1, 100, 100)
...
class BlobFetcher(Process):
    ...
    def _sample_classes(self):
        ...
        for i in xrange(self._num_classes):
            for r in self._roidb:
                if i in r['gt_classes']:
                    self._classes_roidb[i].append(r)
        self._classes_roidb = temp[1:]
