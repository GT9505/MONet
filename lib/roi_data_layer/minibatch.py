...
def annToMask_from_coco(poly,h,w):
    """
...
def get_minibatch(roidb, num_classes):
    ...
    if cfg.TRAIN.IMAGE_BASE_SIZE > 1: # fix the image size to the times of image_base_size
        im_blob, im_scales = _get_image_blob(roidb, random_scale_inds)
    else: # do not fix the image size to the times of image_base_size
        im_blob, im_scales = _get_image_blob(roidb, random_scale_inds)
    ...
    if cfg.TRAIN.HAS_RPN:
        ...
        if cfg.TRAIN.IMAGE_BASE_SIZE > 1:
            gt_boxes[:, 0:4:2] = roidb[0]['boxes'][gt_inds, 0:4:2] * im_scales[0][1]
            gt_boxes[:, 1:4:2] = roidb[0]['boxes'][gt_inds, 1:4:2] * im_scales[0][0]
        ...
        if 0:
            segmentations_poly = roidb[0]['segmentations']
            iscrowds = roidb[0]['iscrowds']
            segmentations = np.zeros( \
                (roidb[0]['height'],roidb[0]['width'],len(segmentations_poly)), \
            dtype=np.float) - 1
            
            for i in range(len(iscrowds)):
                if iscrowds[i] == 0:
                    segmentations[:,:,i] = annToMask_from_coco(segmentations_poly[i],roidb[0]['height'],roidb[0]['width'])
                    if roidb[0]['flipped']:
                        segmentations[:,:,i] = segmentations[:,::-1,i]
                    
            segmentations = cv2.resize(segmentations, dsize=(im_blob.shape[3], im_blob.shape[2]), interpolation=cv2.INTER_NEAREST)
            segmentations = segmentations.reshape((im_blob.shape[2], im_blob.shape[3],len(segmentations_poly)))
            blobs['gt_masks'] = segmentations.transpose( (2,0,1) )
           
        if cfg.TRAIN.MASK_ON:
            segmentations_poly = roidb[0]['segmentations']
            iscrowds = roidb[0]['iscrowds']
            segmentations = np.zeros( \
                (len(segmentations_poly), im_blob.shape[2], im_blob.shape[3]), \
            dtype=np.float)
            
            for i in range(len(iscrowds)):
                if iscrowds[i] == 0:
                    segmentation = annToMask_from_coco(segmentations_poly[i],roidb[0]['height'],roidb[0]['width'])
                    segmentations[i] = cv2.resize(segmentation, dsize=(im_blob.shape[3], im_blob.shape[2]), interpolation=cv2.INTER_NEAREST)
                else:
