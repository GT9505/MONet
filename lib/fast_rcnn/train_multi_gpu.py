...
class SolverWrapper(object):
    ...
    def snapshot(self):
        ...
        scale_bbox_params_monet = (cfg.TRAIN.BBOX_REG and
                                  cfg.TRAIN.BBOX_NORMALIZE_TARGETS and
                                  net.params.has_key('roi_bbox_pred') and
                                  net.params.has_key('psroi_bbox'))
        ...
        if scale_bbox_params_faster_rcnn:
            ...
            net.params['bbox_pred'][0].data[...] = \
                    (net.params['bbox_pred'][0].data *
                     self.bbox_stds.reshape((orig_1.shape[0], 1, 1, 1)))
        ...
        if scale_bbox_params_monet:
        
            # save original values
            orig_0_7 = net.params['psroi_bbox'][0].data.copy()
            orig_1_7 = net.params['psroi_bbox'][1].data.copy()
            repeat_7 = orig_1_7.shape[0] / self.bbox_means.shape[0]

            # scale and shift with bbox reg unnormalization; then save snapshot
            net.params['psroi_bbox'][0].data[...] = \
                (net.params['psroi_bbox'][0].data *
                 np.repeat(self.bbox_stds, repeat_7).reshape((orig_1_7.shape[0], 1, 1, 1)))
            net.params['psroi_bbox'][1].data[...] = \
                (net.params['psroi_bbox'][1].data *
                 np.repeat(self.bbox_stds, repeat_7)) #+ np.repeat(self.bbox_means, repeat_7))

            # save original values
            orig_0_1 = net.params['roi_bbox_pred'][0].data.copy()
            orig_1_1 = net.params['roi_bbox_pred'][1].data.copy()
            repeat_1 = orig_1_1.shape[0] / self.bbox_means.shape[0]

            # scale and shift with bbox reg unnormalization; then save snapshot
            net.params['roi_bbox_pred'][0].data[...] = \
                (net.params['roi_bbox_pred'][0].data *
                 np.repeat(self.bbox_stds, repeat_1).reshape((orig_1_1.shape[0], 1, 1, 1)))
            net.params['roi_bbox_pred'][1].data[...] = \
                (net.params['roi_bbox_pred'][1].data *
        ...
        if scale_bbox_params_monet:
            # restore net to original state
            net.params['psroi_bbox'][0].data[...] = orig_0_7
            net.params['psroi_bbox'][1].data[...] = orig_1_7
            net.params['roi_bbox_pred'][0].data[...] = orig_0_1
            net.params['roi_bbox_pred'][1].data[...] = orig_1_1
        if scale_bbox_params_rpn:
            # restore net to original state
